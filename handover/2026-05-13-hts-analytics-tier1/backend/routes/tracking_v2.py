...
# Heat-to-Trip live map: a torpedo is "currently feeding heat X" if HTS
# recorded a heat for this torpedo within HTS_CURRENT_HEAT_WINDOW_MIN
# whose torpedo_out_time has not yet elapsed (or is NULL). Window kept
# conservative (90 min) because torpedo dwell at SMS is typically <60 min.
HTS_CURRENT_HEAT_WINDOW_MIN = 90


def _current_heat_for_fleet(db: Session, fleet_id: str) -> Optional[dict]:
    """
    Return a slim dict describing the heat this torpedo is currently
    feeding into, or None if no recent HTS row exists.

    Joined data:
      - hts_heat_mirror             — converter_no, sms, heat_no, in/out times
      - h_caster_heat_process_mirror — final_grade, shift, shift_incharge (operator)
      - h_caster_consumption_mirror — yield_pct (NULL until the heat finishes)

    Tier 1 #2 from the HTS analytics roadmap. Folds into the existing
    /torpedoes/{fleet_id} payload so the Live Tracking V2 detail panel
    can render a "FED INTO" section without an extra request.
    """
    cutoff = datetime.utcnow() - timedelta(minutes=HTS_CURRENT_HEAT_WINDOW_MIN)
    hts = (
        db.query(HtsHeatMirror)
        .filter(
            HtsHeatMirror.torpedo_no == fleet_id,
            HtsHeatMirror.torpedo_in_time >= cutoff,
        )
    ...
    proc = (
        ...
        .filter(HCasterHeatProcessMirror.heat_no == hts.heat_no)
        .first()
    )
    cons = (
        db.query(HCasterConsumptionMirror)
        .filter(HCasterConsumptionMirror.heatno == hts.heat_no)
        .first()
    )
    ...
    return {
        ...
        "hotmetal_qty":     float(hts.hotmetal_qty) if hts.hotmetal_qty is not None else None,
        "torpedo_in_time":  hts.torpedo_in_time.isoformat() if hts.torpedo_in_time else None,
        "torpedo_out_time": hts.torpedo_out_time.isoformat() if hts.torpedo_out_time else None,
        "still_pouring":    hts.torpedo_out_time is None,
        "grade":            proc.final_grade   if proc else None,
        "shift":            proc.shift         if proc else None,
        "shift_incharge":   proc.shift_incharge if proc else None,
        "p1_operator":      proc.p1_operator   if proc else None,
        "yield_pct":        float(cons.yield_pct) if cons and cons.yield_pct is not None else None,
    }
...
def torpedo_detail(
    ...
    # Current heat the torpedo is feeding (HTS-side, Tier 1 #2)
    current_heat = _current_heat_for_fleet(db, fleet_id)
    ...
    return {
        ...
        "current_heat": current_heat,            # NEW Tier 1 #2 — null when no recent HTS row
        "chemistry": {
