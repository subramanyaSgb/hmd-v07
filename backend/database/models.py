from sqlalchemy import Column, Integer, String, Float, Numeric, DateTime, Boolean, Date, Text, UniqueConstraint, Index, ForeignKey
...
class FleetLiveLocation(Base):
    ...
    # SuVeechi `vw_unit_status_ist.location` — textual position string
    # (e.g. "At HMY2 - Corex Point No.125", "0.08 KM NW of LRS1"). Captured
    # for the Version 2 Live Tracking detail panel; previously dropped on
    # ingest. Nullable for backwards-compat with rows synced before the
...
class WbatnglTripMirror(Base):
    ...
    # SMS-side actual weights from WBATNGL's `NET_WEIGHT_ACTUAL` /
    # `TARE_WEIGHT_ACTUAL` columns — captured for the Weighbridge Audit
    # page's variance reconciliation (BF reading vs SMS receipt). Added
    # 2026-05-13 (migration wbaudit001); nullable so existing rows pass
    # until the next sync tick re-upserts with the actuals filled in.
...
class HCasterHeatProcessMirror(Base):
    """
    Mirror of HTS.H_CASTER_HEAT_PROCESS (108 cols upstream, ~15.5K rows).
    Populated by hts_sync.py.

    We mirror a curated subset (~25 cols) that matters for HMD use cases:
    heat lifecycle times, operator accountability, delays, and operator
    REMARKS. The rest of upstream's 108 columns (FLUC_POS5, STR2 mirror
    cols, etc.) is caster-internal detail that doesn't surface in V2.

    HEAT_NO is the natural key — joins to HtsHeatMirror.heat_no and to
    HCasterConsumptionMirror.heatno. NB: upstream uses HEATNO (no
    underscore) in consumption table but HEAT_NO (with underscore) here.

    Used by: SMS Performance page (per-heat yield via JOIN with
    consumption mirror), Live Tracking V2 detail panel (operator
    accountability), Trip Operations V2 Exceptions queue (REMARKS feed).
    """
    __tablename__ = "h_caster_heat_process_mirror"

    id              = Column(Integer, primary_key=True)
    heat_no         = Column(String(20), unique=True, nullable=False, index=True)
    sequence_id     = Column(String(20), index=True)                              # SEQUENCE column

    caster_date     = Column(DateTime, index=True)
    shift           = Column(String(2))
    shift_incharge  = Column(String(20))
    p1_operator     = Column(String(20))
    mould_operator  = Column(String(20))
    tcm_operator    = Column(String(20))

    # Ladle lifecycle (continuation of WBATNGL trip)
    ladle_on_turret = Column(DateTime)
    ladle_open      = Column(DateTime)
    ladle_close     = Column(DateTime)

    # Production
    cast_size       = Column(Numeric(10, 3))
    cast_length     = Column(Numeric(10, 3))
    cast_weight     = Column(Numeric(10, 3))
    no_of_slabs     = Column(Integer)
    final_grade     = Column(String(40))

    # Delays
    delay_minutes   = Column(Numeric(10, 2))

    # Operator notes — the "disposition" data the V2 features asked for
    remarks                 = Column(Text)
    liqui_robotic_remarks   = Column(Text)

    # Caster-side slag (for completeness, not BF-side)
    td_slag_depth   = Column(Numeric(10, 3))

    synced_at = Column(DateTime, server_default=func.now())


class HCasterConsumptionMirror(Base):
    """
    Mirror of HTS.H_CASTER_CONSUMPTION (70 cols upstream, ~15.5K rows).
    Populated by hts_sync.py.

    Curated subset of the 70 upstream cols — the ones that power yield
    trends, loss-Pareto breakdowns, and equipment-life tracking. We
    skip strand-2-mirror columns (HEAD_CROP_STR1/STR2, etc.) since they
    duplicate the unsuffixed ones for non-strand-2 plants.

    HEATNO (no underscore — upstream's spelling) is the natural key.
    Joins to HCasterHeatProcessMirror.heat_no and HtsHeatMirror.heat_no.

    Used by: SMS Performance page (YIELD trends + loss Pareto).
    """
    __tablename__ = "h_caster_consumption_mirror"

    id              = Column(Integer, primary_key=True)
    heatno          = Column(String(20), unique=True, nullable=False, index=True)
    sequence_id     = Column(String(20), index=True)

    # The #1 KPI
    yield_pct       = Column(Numeric(7, 3))     # % — e.g. 96.140
    prime_slab      = Column(Numeric(10, 3))

    # Loss breakdown (units = some are %, some are tonnes — sourced as upstream gives)
    ladle_loss      = Column(Numeric(10, 3))
    tun_loss        = Column(Numeric(10, 3))
    head_crop       = Column(Numeric(10, 3))
    tail_crop       = Column(Numeric(10, 3))
    other_loss      = Column(Numeric(10, 3))
    sample_loss     = Column(Numeric(10, 3))
    cut_loss        = Column(Numeric(10, 3))
    mill_scale_loss = Column(Numeric(10, 3))

    # Same losses but in tonnes (more precise for Pareto)
    head_crop_loss_tons     = Column(Numeric(10, 3))
    tail_crop_tons          = Column(Numeric(10, 3))
    sample_loss_tons        = Column(Numeric(10, 3))
    other_loss_tons         = Column(Numeric(10, 3))

    # Consumables
    casting_powder  = Column(String(40))
    cp_consumed     = Column(Numeric(10, 3))
    tun_powder      = Column(Numeric(10, 3))

    # Equipment life counters (refractory wear)
    mbs_life        = Column(Integer)
    sen_life        = Column(Integer)
    shrd_life       = Column(Integer)

    synced_at = Column(DateTime, server_default=func.now())


class HEqupBreakdownMirror(Base):
    """
    Mirror of HTS.H_EQUP_BREAKDOWNS (8 cols upstream, ~800 rows total).
    Populated by hts_sync.py.

    Per-breakdown row. Examples seen in upstream sample:
      REASON='TORPEDO PLACEMENT'       — directly HMD-relevant
      REASON='H1S1 DISPENSER FILLING'  — equipment-side incident

    Natural key is a composite (no single unique upstream column), so
    we use a synthetic auto-PK + a unique index on
    (unit_code, brk_date, reason[:50]) to dedupe on re-sync.

    Used by: V2 Dashboard Alerts feed (folded in via
    `utils/alert_detector.scan_hts_breakdowns()`).
    """
    __tablename__ = "h_equp_breakdown_mirror"

    id              = Column(Integer, primary_key=True)
    eq_code         = Column(Integer)
    unit_code       = Column(Integer, index=True)    # joins to h_unit_code_mirror.unit_code
    reason          = Column(String(255))
    brk_date        = Column(DateTime, index=True)
    brk_date_end    = Column(DateTime)
    brk_shift       = Column(String(2))
    dur_brk_hrs_min = Column(String(40))
    delay_type      = Column(String(20))
    synced_at       = Column(DateTime, server_default=func.now())

    __table_args__ = (
        UniqueConstraint('unit_code', 'brk_date', 'reason',
                         name='_hbr_unit_date_reason_uc'),
        Index('idx_hbr_brk_date_desc', brk_date.desc()),
    )


class HUnitCodeMirror(Base):
    """
    Mirror of HTS.H_UNIT_CODES (2 cols upstream, 36 rows).
    Lookup table — joined to H_EQUP_BREAKDOWNS to translate UNIT_CODE
    integers ("35") into readable names ("CON D", "Mixer").

    Tiny + ~static, but we mirror it so the breakdown feed can show
    human-readable unit names without doing a cross-DB join per row.
    """
    __tablename__ = "h_unit_code_mirror"

    unit_code = Column(Integer, primary_key=True)
    unit_desc = Column(String(80))
    synced_at = Column(DateTime, server_default=func.now())
