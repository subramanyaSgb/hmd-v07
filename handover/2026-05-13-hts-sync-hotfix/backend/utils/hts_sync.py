...
# ------------------------------------------------------------
# NB: SEQUENCE, SHIFT, DELAY are Oracle non-reserved keywords. Most setups
# accept them bare in a SELECT list, but defensively we double-quote them
# here as "SEQUENCE", "SHIFT", "DELAY" so the parser can't be tempted to
# parse them as DDL. The double-quoted name must match the case of the
# upstream column — Oracle stores unquoted DDL names in UPPERCASE.
_CASTER_HP_COLS = (
...
# ------------------------------------------------------------
# SEQUENCE / YIELD are Oracle non-reserved keywords — quote defensively.
_CASTER_CN_COLS = (
...
def pull_breakdowns(db: Session, cursor) -> dict:
    ...
    on the same incident. Postgres's ON CONFLICT DO UPDATE can only
    touch a single conflict-key target row per statement, so a duplicate
    ...
    # Dedupe by (unit_code, brk_date, reason) — last write wins.
    dedup: dict = {}
    for d in mirror_rows:
        dedup[(d["unit_code"], d["brk_date"], d["reason"])] = d
    deduped = list(dedup.values())
    intra_batch_dups = len(mirror_rows) - len(deduped)

    persisted = _generic_upsert(
        db, HEqupBreakdownMirror, deduped,
        ["unit_code", "brk_date", "reason"],
    )
    return {"fetched": len(rows), "upserted": persisted,
            "skipped_incomplete": skipped,
            "intra_batch_dups": intra_batch_dups,
            "watermark": wm}
