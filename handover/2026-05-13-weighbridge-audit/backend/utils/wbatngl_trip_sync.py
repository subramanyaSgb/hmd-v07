...
def row_to_mirror_dict(
    ...
    return {
        ...
        # SMS-side actual weights — previously dropped on ingest, now
        # captured for the Weighbridge Audit page's variance check.
        # Oracle returns NUMBER columns that may arrive as Decimal or
        # 0/None — `_zero_to_null` collapses "no reading yet" to NULL
        # so the variance math doesn't render a fake +100% variance.
        "net_weight_actual":  _zero_to_null(r.get("NET_WEIGHT_ACTUAL")),
        "tare_weight_actual": _zero_to_null(r.get("TARE_WEIGHT_ACTUAL")),
...
def pull_and_upsert_from_source(
    ...
    # Version 2 dashboard — scan the just-upserted rows for chemistry /
    # cold-metal / dwell / no-sms-ack alerts. Detector is dedupe-aware,
    # so re-running over an unchanged row is a no-op. Wrapped in
    # try/except because a detector regression must never break the
    # main sync — alerts are a downstream feature.
    try:
        from .alert_detector import scan_wbatngl_rows
        stats["alerts_added"] = scan_wbatngl_rows(db, mirror_rows)
        if stats["alerts_added"]:
            db.commit()
    except Exception:
        logger.exception("alert_detector.scan_wbatngl_rows failed (non-fatal)")
        try:
            db.rollback()
        except Exception:
            pass
...
def run_once(backfill_days: int = 0) -> dict:
    ...
    total = {"fetched": 0, "upserted": 0,
             "skipped_non_torpedo": 0, "alerts_added": 0, "errors": 0}
