...
const ConsumerStatistics = ({ userId }) => {
    ...
    const handleExportPDF = useCallback(async () => {
        ...
        try {
            ...
            doc.text(`Consumer: ${userId}`, pageWidth / 2, 19, { align: 'center' });
            ...
            doc.setFont('helvetica', 'bold');
            doc.setTextColor(30, 41, 59);
            doc.text('Consumption Progress', margin + 25, y + 8);
            ...
            const perfStats = [
                { label: 'Received', value: `${perf?.total_tonnage || 0} MT`, color: [59, 130, 246] },
            ...
            doc.setFont('helvetica', 'bold');
            doc.setTextColor(30, 41, 59);
            doc.text('My Receipts by Producer', margin, y);
            ...
            doc.save(`Consumer_Dashboard_${userId}_${new Date().toISOString().split('T')[0]}.pdf`);
        } catch (error) {
    ...
    const fetchData = async () => {
        ...
        try {
            const [monthly, perf, partners, time, tripList] = await Promise.all([
                api.get('/api/statistics/my-monthly-plan', { user_id: userId, role: 'consumer' }),
                api.get('/api/statistics/my-performance', { user_id: userId, role: 'consumer' }),
                api.get('/api/statistics/my-partner-breakdown', { user_id: userId, role: 'consumer' }),
                api.get('/api/statistics/my-trip-timeline', { user_id: userId, role: 'consumer' }),
                api.get('/api/statistics/my-trips', { user_id: userId, role: 'consumer' })
            ]);
            ...
            if (import.meta.env.DEV) {
                console.error('Failed to fetch consumer statistics:', err);
            }
    ...
    if (loading && !performance) {
        return (
            <div className="consumer-statistics">
    ...
    return (
        <div className="consumer-statistics compact-layout">
            <div className="statistics-content">
                <MonthlyPlanOverview data={monthlyPlan} userType="consumer" userId={userId} lastUpdated={lastUpdated} onRefresh={fetchData} isRefreshing={loading} />
                <MyPerformanceCard data={performance} userType="consumer" />

                <div className="two-column-section">
                    <PartnerBreakdown data={partnerBreakdown} partnerType="Producer" />
